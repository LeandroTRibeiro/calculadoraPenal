...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md border border-stone-200 bg-white px-3 py-2 text-sm placeholder:text-stone-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:border-stone-800 dark:bg-stone-950 dark:placeholder:text-stone-400",
      className
    )}
    {...props}
  >
    <input
      ...
      aria-hidden="true"
      tabIndex={-1} // Remove from tab order since it's not to be interacted with directly
      value={props.value || ''} // You'll need to manage this value to reflect the selected option
    />
    {children}
